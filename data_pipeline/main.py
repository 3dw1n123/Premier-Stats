...
def get_data() -> tuple[pd.DataFrame,list[pd.DataFrame],pd.DataFrame]:
    ...
    return league_table, dfs,player_understat

def cleaning_data(league_table: pd.DataFrame, dfs: list[pd.DataFrame],understat: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    return clean_league_table, clean_player_stats
...
def main() -> None:
    ...
    """
    #Extract
    league_table, dfs,understat = get_data()
    
    #Transform
    clean_league_table, clean_player_stats = cleaning_data(league_table, dfs, understat)
